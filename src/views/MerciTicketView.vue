    ...
    <div class="page-container container py-5">
      ...
      <div class="ticket-card-container fade-in-up">
        <div class="ticket-preview-card">
          ...
          <div class="copy-ticket-box mt-4">
            <label class="small-label">Code de référence</label>
            <div class="yaourt-field">
              <input type="text" :value="ticketCode" readonly class="code-input" />
      ...
      <div class="info-section mt-5 fade-in">
        <div class="info-card">
          ...
          <div class="faq-item">
            ...
            <p>Une image a été générée automatiquement. <strong>Si le téléchargement n'a pas démarré, cliquez sur le bouton doré
              ci-dessus</strong>. Présentez cette image à l'entrée.</p>
          </div>

          <div class="faq-item">
            <h4>J'ai perdu mon image,  que faire ?</h4>
            <ul>
              <li>Un système d'enregistrement dans le navigateur est prévu et vous permettra de récupérer le dernier ticket acheté depuis la page d'achat de ticket (via un bouton en bas de la page)</li>
              <li>En dernier recours, contactez le service client muni de votre code de référence : <strong>{{ ticketCode }}</strong>.</li>
            </ul>
            <p></p>
          </div>
          ...
          <div class="faq-item">
            <h4>Le ticket</h4>
            <ul>
              <li> Le ticket est unique et scannable une et  <strong style="color:red;" >une seule fois.</strong> </li>
              <li>Conservez bien votre fichier numérique <strong style="color:red;" >et ne compter pas entierement sur votre navigateur pour enregistrer votre ticket </strong></li>
            </ul>
          </div>
        </div>
      </div>
...
const buyerName = ref(localStorage.getItem('event_buyer_name') || 'Invité');
...
const copyToClipboard = async () => {
  try {
    ...
    copied.value = true;
    setTimeout(() => { copied.value = false; }, 2000); // Reset l'icône après 2s
  } catch (err) {
...
.code-input {
  ...
  color: #333;
  font-size: 1rem;
  width: 50%; /* Force la réduction si besoin */
}
